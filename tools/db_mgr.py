...
def dump_data(db_campaign, db_user):
    ...
    subprocess.run(['/root/ET_Dashboard/venv/bin/cqlsh', '-e', f"copy data.cmp{db_campaign.id}_usr{db_user.id} to \'{file_path}\' with header = true;"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    return file_path
