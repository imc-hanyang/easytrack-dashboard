...
def file_is_valid(data_source: mdl.DataSource, fp: TemporaryUploadedFile):
  ...
  columns = []
  type_map = {'timestamp': {int}, 'text': {str}, 'integer': {int}, 'float': {int, float}}
  ...
  for req_col, df_col in column_comparisons:
    ...
    print('expected', f'{expected_col_name}({acceptable_col_types})', 'actual', f'{df_col}({df.dtypes[df_col]})')

    if df_col != expected_col_name or df.dtypes[df_col] not in acceptable_col_types:
      is_valid = False
      break
